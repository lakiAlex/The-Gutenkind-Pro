...
<?php if ($show_logo == true) { ?>
	<a class="header__logo header--style-5 header__logo--big" href="<?php echo esc_url(home_url('/')); ?>">
		<?php if ($logo_type == 'image') { ?>
			<img src="<?php echo esc_url($logo_img); ?>" alt="<?php esc_attr(bloginfo( 'name' )); ?>"/>
		<?php } else { ?>
			<h1><?php bloginfo('name'); ?></h1>
		<?php } ?>
		<?php if ($show_tagline == true) { ?>
			<p class="link"><?php bloginfo('description'); ?></p>
		<?php } ?>
	</a>
<?php } ?>

<header class="header header--style-5">

	<div class="header__top container">
		<div class="header__ham"><div class="header__ham-inner"></div></div>
		<div class="header__nav">
			<?php gutenkind_menu_header(); ?>
		</div>
	...
	<div class="header__nav header__nav--touch">
		<?php
